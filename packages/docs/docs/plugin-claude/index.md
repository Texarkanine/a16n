...
* [Claude Skills](https://docs.anthropic.com/en/docs/claude-code/skills): `.claude/skills/*/SKILL.md`
	...
	* **Skills using non-spec Claude features** → Discovered, content preserved, warned w/ `approximated` (`$ARGUMENTS`, `` !`cmd` ``, `${CLAUDE_*}`, `@path`, and frontmatter keys like `model:` or `argument-hint:` are outside the [AgentSkills.io spec](https://agentskills.io/specification), so their runtime behavior is lost)
