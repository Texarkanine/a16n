...
- [ ] **M4 — Wave B: promote `@a16njs/plugin-cursor`, `@a16njs/plugin-claude`, `@a16njs/plugin-a16n`, and `@a16njs/engine` to `1.0.0`, and re-release `@a16njs/plugin-agentsmd` (already `1.x`) so it re-pins to `@a16njs/models@1.0.0`.** All depend only on the already-published `@a16njs/models@1.0.0` from M3 and have no edges among themselves. — _estimated **L2**_ (coordinated multi-package release that must land after Wave A is live; agentsmd is a dependency-pin refresh, not a `0.x → 1.0.0` promotion).
- [ ] **M5 — Wave C: promote the `a16n` CLI to `1.0.0`.** Re-pins (via pnpm rewrite) to the `1.0.0` engine, plugins (including the M4-refreshed `plugin-agentsmd`), and models published in M3–M4. — _estimated **L1**_ (single package; primarily a forced version bump once its dependencies are all 1.x).
