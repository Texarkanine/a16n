...
Two small follow-ups landed after reflection in response to reviewer feedback on the combined PR (#84, which carries both this task and `20260421-preserve-filename-case`). Both are in-scope for the `path-rewriter` work shipped here and are worth capturing as lessons alongside the above.

- **Empty-string sourcePaths could hang `applyMapping` (r3119786280).** `buildMapping` filtered falsy values in the `sourceItems` fallback branch but not in the `sourcePaths` branch. An empty `sourcePath` would become an empty mapping key; `applyMapping`'s `while ((idx = current.indexOf(sourcePath, idx)) !== -1) { idx += sourcePath.length; }` then loops forever because `indexOf('')` returns `0` and `idx += 0` never advances. Fixed at both layers: `.filter((p) => p.length > 0)` at the `buildMapping` boundary (primary), plus an `if (sourcePath.length === 0) continue;` guard in `applyMapping` (defence-in-depth for any hand-built mapping). Two new tests pin the behaviour (`P27` filters empty explicit entries; `P28` does not fall back to `sourceItems` when explicit entries filter to empty — "explicit REPLACES sourceItems" still holds).
  - **Lesson:** when adding a new code path that parallels an existing defensive one (`sourceItems` fallback had `.filter(Boolean)` from day one), match its defensive posture. `WrittenFile.sourcePaths` is part of `@a16njs/models`' public API; defensive-by-default is the right stance for a field a third-party plugin can populate.

- **CI4 orphan-scope assertion was vacuously true (r3119786251).** The hardening I'd written for Behaviour 8 ("orphan detection should not scan `assets/` or `data/`") put a *mapped* cursor path inside the asset/data payloads and then asserted `not.toMatch(/assets\/|data\//)` against orphan warning messages. Since orphan warnings embed the *found path* in their message (`Orphan path reference: '${foundPath}' ...`), not the file name, and the embedded path was in the mapping anyway, the assertion would have passed even if the scope-skip were removed entirely. Fixed by (a) putting genuinely **unmapped** cursor refs (`cursor/rules/missing-from-asset.mdc`, `…/missing-from-data.mdc`) in the asset/data payloads and (b) asserting those specific strings don't appear in any orphan warning message.
  - **Lesson:** when writing a "scope skip should not fire" test, make sure the positive case **would** fire if the scope skip were removed. Otherwise it's a test that validates its own setup, not the production behaviour. A quick mental "invert the production flag" pass on the test payload would have caught this.

Both fixes ship in a single follow-up commit on the same PR. They don't change the shape of the original design — they tighten the boundary of the helpers already introduced.
