...
- **QA** — Complete. One doc accuracy fix applied: CONTRIBUTING.md note incorrectly implied `@a16njs/glob-hook` was still a special case requiring `pnpm exec`; fixed to reflect that the cwd fix makes all packages work from root with `npx vitest run`. All 12 tests still pass. → entering REFLECT.
- **REFLECT** — Complete. Reflection captured technical insight about `npx` binary resolution in pnpm monorepos and process insight about writing docs from the post-fix perspective. Persistent files scan: no further updates needed beyond what was already done in BUILD. → ready for ARCHIVE.
