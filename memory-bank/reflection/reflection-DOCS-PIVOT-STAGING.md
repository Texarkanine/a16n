...
**Note:** Phases 4-6 implementation is complete but not yet committed.
...
| Wrapper MDX pages | Provide context and curation, not just raw TypeDoc |
...
| TypeDoc generation time (versioned) | N/A | ~4 minutes (all tags) |
| Total build time | N/A (broken) | ~195s (dev) / ~4min (CI) |
...
- `packages/docs/package.json` - Added versioned scripts, dual build modes
- `packages/docs/docusaurus.config.js` - Added `routeBasePath: '/'`, custom `sidebarItemsGenerator`
